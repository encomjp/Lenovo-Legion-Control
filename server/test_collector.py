...
Verifies:
1. Gzip compressed bodies (the client's actual wire format via `curl -H
   Content-Encoding: gzip --data-binary @-`) are decompressed and accepted.
2. `x-gzip` is accepted as an alias for `gzip` (common proxy behavior).
3. Plain (uncompressed) bodies continue to work unchanged.
4. The database stores the gunzipped, valid JSON string verbatim — never
   re-serialized, never the compressed bytes.
5. Decompression bombs (payload expands > MAX_BODY_BYTES) return 413.
6. Corrupt gzip streams return 400, not an unhandled 500 error.
7. Unknown schema versions and non-dict JSON return 400.
8. Shared-secret authentication (X-Legion-Telemetry-Key).
9. Rate limiting (sliding window per IP).
10. Automatic retention pruning.
"""
...
def _payloads_like(fragment: str) -> list[str]:
    ...
    try:
        rows = conn.execute(
            "SELECT payload FROM reports WHERE payload LIKE ?",
            (f"%{fragment}%",),
        ).fetchall()
    finally:
        conn.close()
...
def test_gzipped_push_is_accepted(client):
    ...
    r = _post(client, compressed, **{"Content-Encoding": "gzip"})
...
def test_stored_payload_is_verbatim_gunzipped_json(client):
    ...
    assert len(rows) == 1
    # DB must hold valid JSON, identical to what was compressed on the client,
    # not the raw gzip container or a re-serialized variant.
...
def test_corrupt_gzip_is_400_not_500(client):
    before = _row_count()
    r = _post(client, b"\x1f\x8b\x08 not a real gzip", **{"Content-Encoding": "gzip"})
    assert r.status_code == 400, r.text
    assert "corrupt gzip" in r.text
...
def test_decompression_bomb_is_413(client):
    ...
    assert "decompressed payload too large" in r.text
    assert _row_count() == before
